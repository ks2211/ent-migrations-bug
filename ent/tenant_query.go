...
func (tq *TenantQuery) QueryGroups() *GroupsQuery {
	query := &GroupsQuery{config: tq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (tq *TenantQuery) WithGroups(opts ...func(*GroupsQuery)) *TenantQuery {
	query := &GroupsQuery{config: tq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (tq *TenantQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tenant, error) {
	...
	_spec.Node.Schema = tq.schemaConfig.Tenant
...
func (tq *TenantQuery) sqlQuery(ctx context.Context) *sql.Selector {
	...
	t1.Schema(tq.schemaConfig.Tenant)
	ctx = internal.NewSchemaConfigContext(ctx, tq.schemaConfig)
