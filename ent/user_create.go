...
func (uc *UserCreate) AddUserPermIDs(ids ...int) *UserCreate {
	uc.mutation.AddUserPermIDs(ids...)
	return uc
}

// AddUserPerms adds the "user_perms" edges to the UserPerms entity.
func (uc *UserCreate) AddUserPerms(u ...*UserPerms) *UserCreate {
	ids := make([]int, len(u))
	for i := range u {
		ids[i] = u[i].ID
	}
	return uc.AddUserPermIDs(ids...)
}
...
func (uc *UserCreate) createSpec() (*User, *sqlgraph.CreateSpec) {
	...
	if nodes := uc.mutation.UserPermsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
		...
		edge.Schema = uc.schemaConfig.UserPerms
		for _, k := range nodes {
			edge.Target.Nodes = append(edge.Target.Nodes, k)
		}
		_spec.Edges = append(_spec.Edges, edge)
