...
func (uq *UserQuery) QueryUserPerms() *UserPermsQuery {
	query := &UserPermsQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withUserPerms: uq.withUserPerms.Clone(),
...
func (uq *UserQuery) WithUserPerms(opts ...func(*UserPermsQuery)) *UserQuery {
	query := &UserPermsQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withUserPerms = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withUserPerms; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.UserPerms = []*UserPerms{}
		}
		query.Where(predicate.UserPerms(func(s *sql.Selector) {
			s.Where(sql.InValues(user.UserPermsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			...
			node.Edges.UserPerms = append(node.Edges.UserPerms, n)
		}
	}
